...
<?php
// -----------------------------------------------------
// SECTION 4 — QUIZ RETEST PERIOD MANAGEMENT
// -----------------------------------------------------
...
<div class="card analytics-card mb-4">
    <div class="card-header analytics-header">
        <h5 class="mb-0">🔄 Quiz Retest Period Management</h5>
    </div>
    <div class="card-body">
        <p class="mb-3">Manage retest periods for quizzes. Set periodic requirements for users to retake completed quizzes.</p>

        <?php
        try {
            // Fetch all quizzes with their retest periods and statistics
            $quiz_stmt = $pdo->query("
                SELECT
                    tq.id,
                    tq.quiz_title,
                    COALESCE(tq.retest_period_months, 0) as retest_period,
                    p.title as post_title,
                    COUNT(DISTINCT uqa.user_id) as total_attempts,
                    COUNT(DISTINCT CASE
                        WHEN uqa.status = 'passed'
                        AND uqa.completed_at IS NOT NULL
                        AND tq.retest_period_months > 0
                        AND DATE_ADD(uqa.completed_at, INTERVAL tq.retest_period_months MONTH) <= CURDATE()
                        THEN uqa.user_id
                    END) as users_needing_retest
                FROM training_quizzes tq
                LEFT JOIN posts p ON tq.content_id = p.id
                    AND LOWER(COALESCE(tq.content_type, '')) IN ('post', '')
                LEFT JOIN user_quiz_attempts uqa ON tq.id = uqa.quiz_id
                    AND uqa.status IN ('passed', 'failed')
                GROUP BY tq.id, tq.quiz_title, tq.retest_period_months, p.title
                ORDER BY p.title, tq.quiz_title
            ");

            $quizzes = $quiz_stmt->fetchAll(PDO::FETCH_ASSOC);

            if (empty($quizzes)) {
                echo "<div class='alert alert-info'>No quizzes found in the system.</div>";
            } else {
        ?>
        ...
        <div class="table-responsive analytics-table-wrapper">
            <table class="table table-bordered table-striped table-hover align-middle analytics-table mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo htmlspecialchars($quiz['quiz_title'] ?: 'Quiz #' . $quiz['id']); ?></td>
                        <td><?php echo htmlspecialchars($quiz['post_title'] ?: 'Standalone Quiz'); ?></td>
                        <td>
                            <span class="badge <?php echo $quiz['retest_period'] > 0 ? 'bg-warning' : 'bg-secondary'; ?>">
                                <?php
                                if ($quiz['retest_period'] > 0) {
                                    echo $quiz['retest_period'] . ' month' . ($quiz['retest_period'] != 1 ? 's' : '');
                                } else {
                                    echo 'No retest';
                                }
                                ?>
                        ...
                        <td>
                            <?php
                            if ($quiz['users_needing_retest'] > 0) {
                                echo "<span class='badge bg-danger'>" . $quiz['users_needing_retest'] . " users</span>";
                            } else {
                        ...
                        <td>
                            <button class="btn btn-xs btn-primary" onclick="openRetestModal(<?php echo $quiz['id']; ?>, '<?php echo htmlspecialchars($quiz['quiz_title'] ?: 'Quiz #' . $quiz['id']); ?>', <?php echo $quiz['retest_period']; ?>)">
                                Set Period
                            </button>
                        </td>
                    </tr>
        ...
        <div class="mt-3">
            <button class="btn" onclick="processRetestPeriods()">
                🔄 Process Retest Periods
            </button>
            <small class="text-muted ms-2">Check all completed quizzes and mark users who need to retake based on retest periods</small>
        </div>

        <?php
            }
        } catch (PDOException $e) {
            echo "<div class='alert alert-danger'>Error loading quiz data: " . htmlspecialchars($e->getMessage()) . "</div>";
        }
        ?>
    </div>
</div>
...
<!-- Retest Period Modal -->
<div class="modal fade" id="retestModal" tabindex="-1" aria-labelledby="retestModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none;">
                <h5 class="modal-title" id="retestModalLabel">Set Quiz Retest Period</h5>
            ...
            <div class="modal-body">
                <form id="retestForm">
                    ...
                    <div class="mb-3">
                        <label for="quizTitle" class="form-label">Quiz:</label>
                        <input type="text" class="form-control" id="quizTitle" readonly>
                    </div>

                    <div class="mb-3">
                        <label for="retestPeriod" class="form-label">Retest Period:</label>
                        <select class="form-select" id="retestPeriod" name="retest_period_months" required>
                        ...
                        <div class="form-text">Select how often users must retake this quiz. "No retest required" means once passed, users never need to retake it.</div>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn" onclick="saveRetestPeriod()">Save Changes</button>
            </div>
        </div>
    </div>
</div>
...
<script>
function openRetestModal(quizId, quizTitle, currentPeriod) {
    document.getElementById('quizId').value = quizId;
    document.getElementById('quizTitle').value = quizTitle;
    document.getElementById('retestPeriod').value = currentPeriod;

    const modal = new bootstrap.Modal(document.getElementById('retestModal'));
    modal.show();
}

function saveRetestPeriod() {
    const form = document.getElementById('retestForm');
    const formData = new FormData(form);

    fetch('training_admin_analytics.php', {
        method: 'POST',
        body: formData,
        headers: {
    ...
    .then(data => {
        if (data.success) {
            alert('Retest period updated successfully!');
            location.reload();
        } else {
            alert('Error updating retest period: ' + data.message);
        }
    })
    .catch(error => {
        console.error('Error:', error);
        alert('Error updating retest period');
    });
}

function processRetestPeriods() {
    if (confirm('This will check all completed quizzes and mark users who need to retake based on their retest periods. Continue?')) {
        fetch('training_admin_analytics.php', {
            method: 'POST',
            body: JSON.stringify({action: 'process_retest_periods'}),
            headers: {
        ...
        .then(data => {
            if (data.success) {
                alert('Retest periods processed successfully! ' + data.users_updated + ' users marked for retest.');
                location.reload();
            } else {
                alert('Error processing retest periods: ' + data.message);
            }
        })
        .catch(error => {
            console.error('Error:', error);
            alert('Error processing retest periods');
        });
    }
}
</script>
...
<?php
// Handle AJAX requests for retest period management
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    header('Content-Type: application/json');

    try {
        // Handle retest period save
        if (isset($_POST['quiz_id']) && isset($_POST['retest_period_months'])) {
            $quiz_id = intval($_POST['quiz_id']);
            $retest_period = intval($_POST['retest_period_months']);

            $update_stmt = $pdo->prepare("
                UPDATE training_quizzes
                SET retest_period_months = ?
                WHERE id = ?
            ");
            $update_stmt->execute([$retest_period, $quiz_id]);

            echo json_encode(['success' => true, 'message' => 'Retest period updated']);
            exit;
        }

        // Handle retest period processing
        $input = file_get_contents('php://input');
        $data = json_decode($input, true);

        if (isset($data['action']) && $data['action'] === 'process_retest_periods') {
            // Find users who need to retake quizzes based on retest periods
            $retest_stmt = $pdo->prepare("
                UPDATE user_quiz_attempts uqa
                JOIN training_quizzes tq ON uqa.quiz_id = tq.id
                SET uqa.retest_required = TRUE,
                    uqa.last_completed_date = uqa.completed_at
                WHERE uqa.status = 'passed'
                  AND tq.retest_period_months > 0
                  AND uqa.completed_at IS NOT NULL
                  AND DATE_ADD(uqa.completed_at, INTERVAL tq.retest_period_months MONTH) <= CURDATE()
                  AND uqa.retest_required = FALSE
            ");
            $retest_stmt->execute();
            $users_updated = $retest_stmt->rowCount();

            echo json_encode(['success' => true, 'message' => 'Retest periods processed', 'users_updated' => $users_updated]);
            exit;
        }
