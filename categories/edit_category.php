            ...
            if ($is_admin) {
                if ($departments_column_exists) {
                    $stmt = $pdo->prepare("
                        ...
                        WHERE id = ?
                    ");
                    $stmt->execute([$name, $icon, $visibility, $allowed_users_json, $allowed_departments_json, $visibility_note, $category_id]);
                } else {
                    $stmt = $pdo->prepare("
                        UPDATE categories SET
                            name = ?,
            ...
            <form method="POST" action="edit_category.php?id=<?php echo $category_id; ?>">
                ...
                <?php if ($is_admin): ?>
                    ...
                    if (!empty($category['allowed_departments'])) {
                        $selected_departments = json_decode($category['allowed_departments'], true) ?? [];
                    }
                    ?>
